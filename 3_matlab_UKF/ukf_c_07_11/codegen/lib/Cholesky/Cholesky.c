...
void Cholesky(double A[1024])
{
  ...
  for (k = 0; k < 32; k++) {
    ...
    for (i11 = 0; i11 < loop_ub; i11++) {
      A_data[i11] = A[((i8 + i11) + (k << 5)) - 1] / b_A;
    }

    loop_ub = i9 - i8;
    for (i8 = 0; i8 < loop_ub; i8++) {
      A[((i10 + i8) + (k << 5)) - 1] = A_data[i8];
    }
    ...
    for (j = 0; j <= 30 - k; j++) {
      ...
      loop_ub = i9 - i8;
      for (i11 = 0; i11 <= loop_ub; i11++) {
        b_A_data[i11] = A[(i8 + i11) + ((b_j - 1) << 5)] - b_A * A[(i10 + i11) +
          (k << 5)];
      }

      loop_ub = (i9 - i8) + 1;
      for (i8 = 0; i8 < loop_ub; i8++) {
        A[((b_j + i8) + ((b_j - 1) << 5)) - 1] = b_A_data[i8];
      }
    }
  }
}
