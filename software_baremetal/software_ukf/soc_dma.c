...
#define UKF_ADDRESS ((uint8_t*)FPGA_OCR_ADDRESS_UP + (4*(0x00000001))
	  ...
	  //--Dsdos randomicos--//
		...
		for(int i = 0; i < DMA_TRANSFER_SIZE/4; i++){
			for(int j =0; j<4; j++) {
				if (i<1) {
					if (j==3) {
							DMA_TRANSFER_SRC_UP[(4*i)+j] = (uint8_t)0b00001100; //4
...
/*		printf("Wait \n\r");
		for(long int h=0; h<100000000; h++){};
		 print_src_dst(DMA_TRANSFER_SRC_UP, DMA_TRANSFER_DST_UP, DMA_TRANSFER_SIZE);*/

	//while(fpga_dma_transfer_done(FPGA_DMAC_ADDRESS)==0) {	}
		//pmu_counter_read_ns(&as);
	while(ukf_transfer_done(UKF_DONE_ADDRESS)!=14593297) {}
...
{
  int i;
  //uint32_t* char_ptr_scr = (uint32_t*) src;
  //uint32_t* char_ptr_dst= (uint32_t*) dst;

  printf( "Source=[");
  for (i=0; i<(size/4); i++)
  {
      //printf("%xx %xx %xx %xx,", *(char_ptr_dst),*(char_ptr_dst+1),*(char_ptr_dst+2),*(char_ptr_dst+3));
			printf("%08lxh ", *((uint32_t*) (src+(4*i))));
      if (i<((size/4)-1)) printf(",");
  }
  printf("]\n\r");
