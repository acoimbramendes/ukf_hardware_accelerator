 ...
 * Return Type  : void
 */
unsigned long long b_sigmas(const double x[32], const double P[1024], double X[2080])
{
  double A[1024];
  int i0;
  double Y[1024];
  int i1;
  unsigned long long a1,a2,c1;
  memcpy(&A[0], &P[0], sizeof(double) << 10);
  pmu_counter_read_ns(&a1);
  Cholesky(A);
  pmu_counter_read_ns(&a2);
  c1=a2-a1;
